...
type KrknAISummary struct {
	...
	SuccessfulScenarios int      `json:"successfulScenarios"`
...
func (a *KrknAIAggregator) Collect(ctx context.Context, resultsDir string) (*KrknAIData, error) {
	...
	// Collect config summary (first 50 lines)
	...
	a.logger.Info("completed krkn-ai artifact collection",
		...
		"failedScenarios", data.Summary.FailedScenarios,
...
func (a *KrknAIAggregator) parseScenarioRecord(record []string) (ScenarioResult, error) {
	...
	healthCheckFailure, _ := strconv.ParseFloat(record[4], 64)
	healthCheckResponseTime, _ := strconv.ParseFloat(record[5], 64)
	krknFailure, _ := strconv.ParseFloat(record[6], 64)
	...
	return ScenarioResult{
		...
		Scenario:                     record[2],
		Parameters:                   record[3],
		HealthCheckFailureScore:      healthCheckFailure,
		HealthCheckResponseTimeScore: healthCheckResponseTime,
		KrknFailureScore:             krknFailure,
...
// Uses typed structs to ensure consistent extraction regardless of YAML key order.
...
func (a *KrknAIAggregator) collectLogArtifacts(resultsDir string, data *KrknAIData) error {
	...
	return filepath.Walk(absResultsDir, func(path string, info os.FileInfo, err error) error {
		...
		// Skip PNG files - not useful for LLM text analysis

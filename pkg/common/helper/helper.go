...
func (h *H) CurrentProject() string {
	...
	if viper.GetString(config.Project) != "" {
		_ = wait.PollUntilContextTimeout(context.Background(), 5*time.Second, 30*time.Second, false, func(ctx context.Context) (bool, error) {
			h.proj, err = h.Project().ProjectV1().Projects().Get(ctx, viper.GetString(config.Project), metav1.GetOptions{})

...
func (CcsAwsSession *ccsAwsSession) getClient() (*session.Session, *iam.IAM, *ec2.EC2) {
	var err error

	CcsAwsSession.once.Do(func() {
		populateAwsKeys()
		CcsAwsSession.session, err = session.NewSession(aws.NewConfig().
			WithCredentials(credentials.NewStaticCredentials(accessKey, secretAcccessKey, "")).
...
func VerifyCCS() (string, error) {
	var err error
	CcsAwsSession.session, CcsAwsSession.iam, CcsAwsSession.ec2 = CcsAwsSession.getClient()
...
func populateAwsKeys() {
	//If provider is Rosa, grab the keys
	if viper.GetString(config.Provider) == "rosa" {
		accessKey = viper.GetString("rosa.awsAccessKey")
		secretAcccessKey = viper.GetString("rosa.awsSecretAccessKey")
	} else {
		//If provider is AWS, grab the keys from the ccsKeys variable
		accessKey = viper.GetString("ocm.aws.accessKey")
		secretAcccessKey = viper.GetString("ocm.aws.secretKey")
	}
}

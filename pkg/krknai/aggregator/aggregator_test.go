...
func TestKrknAIAggregator_Collect(t *testing.T) {
	...
	assert.Equal(t, 4, data.Summary.SuccessfulScenarioCount)
	...
	assert.Contains(t, data.ConfigSummary, "=== Enabled Chaos Scenarios ===")
...
func TestKrknAIAggregator_EmptyDirectory(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, data)
	assert.Equal(t, 0, data.Summary.TotalScenarioCount)
}
...
wait_duration: 90`
	...
	assert.Contains(t, data.ConfigSummary, "pod_scenarios")

...
func init() {
	...
	_ = viper.BindEnv(StartOfTimeWindowInHours, "REPORTING_WEATHER_START_OF_TIME_WINDOW_IN_HOURS")

	viper.SetDefault(NumberOfSamplesNecessary, 3)
	_ = viper.BindEnv(NumberOfSamplesNecessary, "REPORTING_WEATHER_NUMBER_OF_SAMPLES_NECESSARY")

	viper.SetDefault(JobAllowlist, "osde2e-.*-aws-e2e-.*")
	_ = viper.BindEnv(JobAllowlist, "REPORTING_WEATHER_JOB_ALLOWLIST")

	viper.SetDefault(Provider, "aws")
	_ = viper.BindEnv(Provider, "REPORTING_WEATHER_PROVIDER")
}

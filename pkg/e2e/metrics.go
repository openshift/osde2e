...
func NewMetrics() *Metrics {
	...
	metadataGatherer := prometheus.NewGaugeVec(
		...
		[]string{"install_version", "upgrade_version", "environment", "phase", "metadata_name", "cluster_id", "job_id"},
	)
	...
	for k, v := range jsonOutput {
		...
		default:
			...
			if floatValue, err := strconv.ParseFloat(stringValue, 64); err == nil {
				gatherer.WithLabelValues(state.Cluster.Version, state.Upgrade.ReleaseName, cfg.OCM.Env, phase, metadataName, state.Cluster.ID, cfg.JobID).Add(floatValue)
			}
		}
	}
}

...
var _ = ginkgo.Describe("Ad Hoc Test Images", ginkgo.Ordered, ginkgo.ContinueOnFailure, label.AdHocTestImages, func() {
	var (
		...
		exeConfig        = &executor.Config{
			...
			OutputDir:           filepath.Join(viper.GetString(config.ReportDir), viper.GetString(config.Phase)),
			...
			Timeout:             viper.GetDuration(config.Tests.PollingTimeout),
		}
		exe *executor.Executor
	)

	if viper.IsSet(config.Tests.AdHocTestContainerTimeout) {
		exeConfig.Timeout = viper.GetDuration(config.Tests.AdHocTestContainerTimeout)
	}

...
var _ = ginkgo.Describe("[Suite: informing] [OSD] dedicated-admin permissions", func() {
	ginkgo.Context("dedicated-admin group permissions", func() {
		...
		ginkgo.It("cannot add members to cluster-admin", func() {
			...
			daGroup, err := h.User().UserV1().Groups().Get("dedicated-admins", metav1.GetOptions{})
			Expect(err).NotTo(HaveOccurred())

			daGroup.Users = append(daGroup.Users, "new-dummy-admin@redhat.com")
			_, err = h.User().UserV1().Groups().Update(daGroup)
		...
		ginkgo.It("cannot delete members from cluster-admin", func() {
			...
			daGroup, err = h.User().UserV1().Groups().Update(daGroup)
			Expect(err).NotTo(HaveOccurred())

...
func TestClusterInteraction(t *testing.T) {
	mockProvider, _ := New(makeConfig("mockEnv"))
...
func TestIntentionalFailures(t *testing.T) {
	...
	mockProvider, _ := New(makeConfig("fail"))
...
func TestSetVersionList(t *testing.T) {
	...
	mockProvider.(*MockProvider).SetVersionList(versionList)
...
func makeConfig(env string) *config.Config {
	return &config.Config{
		OCM: config.OCMConfig{
			Env: env,
		},
	}
}

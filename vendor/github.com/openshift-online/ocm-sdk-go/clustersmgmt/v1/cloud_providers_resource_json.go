...
func readCloudProvidersListRequest(request *CloudProvidersListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeCloudProvidersListResponse(response *CloudProvidersListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(CloudProviderListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeCloudProviderList(response.items.items, stream)
			count++
		}
	}

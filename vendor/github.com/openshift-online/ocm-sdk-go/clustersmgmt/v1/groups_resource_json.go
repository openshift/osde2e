...
func readGroupsListRequest(request *GroupsListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeGroupsListResponse(response *GroupsListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(GroupListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeGroupList(response.items.items, stream)
			count++
		}
	}

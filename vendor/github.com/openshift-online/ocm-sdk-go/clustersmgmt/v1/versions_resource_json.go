...
func readVersionsListRequest(request *VersionsListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeVersionsListResponse(response *VersionsListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(VersionListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeVersionList(response.items.items, stream)
			count++
		}
	}

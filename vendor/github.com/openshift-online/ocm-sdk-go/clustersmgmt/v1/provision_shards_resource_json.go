...
func readProvisionShardsListRequest(request *ProvisionShardsListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeProvisionShardsListResponse(response *ProvisionShardsListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(ProvisionShardListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeProvisionShardList(response.items.items, stream)
			count++
		}
	}

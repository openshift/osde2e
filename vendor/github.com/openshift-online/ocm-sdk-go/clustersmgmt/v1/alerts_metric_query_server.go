...
// dispatchAlertsMetricQuery navigates the servers tree rooted at the given server
// till it finds one that matches the given set of path segments, and then invokes
// the corresponding server.
func dispatchAlertsMetricQuery(w http.ResponseWriter, r *http.Request, server AlertsMetricQueryServer, segments []string) {
	if len(segments) == 0 {
		switch r.Method {
		case "GET":
			adaptAlertsMetricQueryGetRequest(w, r, server)
			return
		default:
...
// adaptAlertsMetricQueryGetRequest translates the given HTTP request into a call to
// the corresponding method of the given server. Then it translates the
// results returned by that method into an HTTP response.
func adaptAlertsMetricQueryGetRequest(w http.ResponseWriter, r *http.Request, server AlertsMetricQueryServer) {
	request := &AlertsMetricQueryGetServerRequest{}
	err := readAlertsMetricQueryGetRequest(request, r)
	if err != nil {
		glog.Errorf(
	...
	response := &AlertsMetricQueryGetServerResponse{}
	response.status = 200
	...
	err = writeAlertsMetricQueryGetResponse(response, w)
	if err != nil {
		glog.Errorf(

...
func readRegistriesListRequest(request *RegistriesListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeRegistriesListResponse(response *RegistriesListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(RegistryListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeRegistryList(response.items.items, stream)
			count++
		}
	}

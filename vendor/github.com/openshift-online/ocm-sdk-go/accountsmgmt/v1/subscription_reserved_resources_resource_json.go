...
func readSubscriptionReservedResourcesListRequest(request *SubscriptionReservedResourcesListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeSubscriptionReservedResourcesListResponse(response *SubscriptionReservedResourcesListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(ReservedResourceListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeReservedResourceList(response.items.items, stream)
			count++
		}
	}

...
CLOUD_PROVIDER_REGION:  Must be a valid region enabled within OCM. Example -> us-east-1
 
CLUSTER_VERSION:  Must be a valid clusterimageset enabled within that environment. Example -> openshift-v4.6.0-fc.3-fast
...
PROVIDER: Provider is what provider to use to create/delete clusters. Example -> ocm, moa, crc, mock
...
BASE_JOB_URL: BaseJobURL is the root location for all job artifacts
For example, https://storage.googleapis.com/origin-ci-test/logs/osde2e-prod-gcp-e2e-next/61/build-log.txt would be https://storage.googleapis.com/origin-ci-test/logs -- This is also our default.
...
ADDON_TEST_USER:  TestUser is the OpenShift user that the tests will run as. If "%s" is detected in the TestUser string, it will evaluate that as the project namespace

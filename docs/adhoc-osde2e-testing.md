...
[OSDe2e jobs](https://github.com/openshift/osde2e/blob/main/docs/CI-Jobs.md) are defined in code. However, ad hoc, parameterized OSDe2e jobs may be run without committing code. This may be useful for testing a new cloud region, a new instance type or validating a particular issue.
...
1. Click Login if necessary (button in upper-right corner)
1. Click "Run" to create a new osde2e build
1. Review and update parameters as needed. See "Test Scenarios" below.
1. Click "Run" to initiate the with parameters build
...
* Instance Type/Machine Type: For instance type testing: update the instance type, e.g. `x1e.xlarge`, otherwise leave empty to select random instance type.
